...
def addproducts(request):
    ...
    if request.POST:
        ...
        product = Products.objects.create(
            name=name, quantity=quantity, price=price, category=Cid, fid=id, img=img
        )
        product.save()
        return redirect("/viewcategory")
    return render(
        request, "farmer/addproducts.html", {"category": category, "farmer": farmer}
    )
...
def farmtechviewloan(request):
    ...
    # view farmtech
    print("Farmtech : ", Uid.id)

    view = Loans.objects.filter(ftid=Uid.id)
    ...
    if request.method == "POST":
        ...
        if img:
            ins = Loans.objects.create(name=name, img=img, url=url, desc=desc, ftid=Uid)
            ins.save()
            return HttpResponse(
                "<script>alert('Load Details Added');window.location='/farmtechviewloan'</script>"
            )
        else:
            ins = Loans.objects.create(name=name, url=url, desc=desc, ftid=Uid)
            ins.save()
            return HttpResponse(
                "<script>alert('Load Details Added');window.location='/farmtechviewloan'</script>"
            )

    return render(request, "farmtech/viewloans.html", {"view": view})


def farmerviewloan(request):
    view = Loans.objects.all()
    return render(request, "farmer/viewloans.html", {"view": view})
...
def addvehicle(request):
    ...
    if request.POST:
        ...
        vehicle = Vehicle.objects.create(
            type=type, amount=amount, img=img, duration=duration, techid=farm
        )
        vehicle.save()
...
def addland(request):
    ...
    if request.POST:
        ...
        img = request.FILES["img"]
        duration = request.POST["duration"]
        land = Lands.objects.create(
            area=area, amount=amount, img=img, duration=duration, techid=farm
        )
        land.save()
        messages.info(request, "added succesfully")
        return redirect("/addrent")
...
#     vehicle=Bookvehiclerent.objects.filter(id=vid).delete()
...
def bookrent(request):
    ...
    if request.POST:
        ...
        print(price)
        rent = Bookvehiclerent.objects.create(
            totalamount=price,
...
def booklandrent(request):
    ...
    if request.POST:
        ...
        price = int(date) * int(amount)
        rent = Booklandrent.objects.create(
            totalamount=price,
            startingdate=startdate,
            endingdate=enddate,
            techid=Uid,
...
def farmer_view_loan(request):
    uid = request.session["uid"]
    Uid = Farmer.objects.get(farmer=uid)
    view = Loan.objects.filter(fid=Uid)
    return render(request, "farmer/view_loan_status.html", {"view": view})
...
def user_reg(request):
    if request.method == "POST":
        ...
        if Login.objects.filter(email=email, password=password).exists():
            return HttpResponse(
                "<script>alert('Email is Already in use');window.location='/login'</script>"
            )
        else:
            ...
            user.save()
            return HttpResponse(
                "<script>alert('User Registered Successfully');window.location='/login'</script>"
            )
...
def user_view_products(request):
    uid = request.session["uid"]
    Uid = User_reg.objects.get(loginid=uid)
    ...
    if request.POST:
        ...
        print("farmer: ", fid)
        Fid = Farmer.objects.get(id=fid)

        pid = request.POST.get("pid")
        Pid = Products.objects.get(id=pid)

        tamount = int(qty) * int(price)

        ins = Purchase.objects.create(
            pid=Pid, uid=Uid, fid=Fid, amount=tamount, qty=qty, status="Ordered"
        )
        ins.save()
        return HttpResponse(
            f"<script>alert('Product Booked Make Payment');window.location='/user_make_payment?pid={ins.id}'</script>"
        )
    return render(request, "User/viewproducts.html", {"products": products})


def user_make_payment(request):
    purchase_id = request.GET.get("pid")
    Pr = Purchase.objects.get(id=purchase_id)

    if request.POST:
        Set = Purchase.objects.filter(id=purchase_id).update(status="Payment Paid")
        return HttpResponse(
            "<script>alert('Payment paid');window.location='/user_view_products'</script>"
        )

    return render(request, "User/payment.html", {"products": Pr})
...
def my_bookings(request):
    ...
    fil = Purchase.objects.filter(uid=Uid)

    return render(request, "User/my_bookings.html", {"view": fil})


def delete_my_bookings(request):
    bd = request.GET.get("bid")
    Dele = Purchase.objects.filter(id=bd).delete()
    return HttpResponse(
        "<script>alert('Delete');window.location='/my_bookings'</script>"
    )
...
def farmer_view_bookings(request):
    ...
    fil = Purchase.objects.filter(fid=Uid)

    return render(request, "farmer/my_bookings.html", {"view": fil})


def farmer_profile(request):
    uid = request.session["uid"]
    view = Farmer.objects.filter(farmer=uid)
    print("view :", view)
    ...
    if request.method == "POST":
        ...
        if image:
            userid = Farmer.objects.get(farmer=uid)
            userid.img = image
            userid.save()

            up = Farmer.objects.filter(farmer=uid).update(
                name=name, email=email, mob=phone, address=address
            )

            pas_up = Login.objects.get(id=uid)
            pas_up.set_password(password)
            pas_up.ViewPassword = password
            pas_up.save()

            HttpResponse(
                "<script>alert('profile updated');window.location='/farmer_profile'</script>"
            )
    ...
    return render(request, "farmer/profile.html", {"view": view})
